## Nexys Video FPGA Board Constraints (XDC) - Stabilized Version

# 100 MHz Differential System Clock (Pins R4/T4)
set_property -dict { PACKAGE_PIN R4    IOSTANDARD DIFF_HSTL_II_18 } [get_ports { clk_100mhz_p }];
set_property -dict { PACKAGE_PIN T4    IOSTANDARD DIFF_HSTL_II_18 } [get_ports { clk_100mhz_n }];

# Timing constraint for the 100MHz Input
create_clock -add -name sys_clk_pin -period 10.00 -waveform {0 5} [get_ports { clk_100mhz_p }];

# Reset button (CPU_RESET - Pin G4)
set_property -dict { PACKAGE_PIN G4    IOSTANDARD LVCMOS15 } [get_ports { reset_btn }];

# UART TX (USB-UART Bridge - Pin AA19)
set_property -dict { PACKAGE_PIN AA19  IOSTANDARD LVCMOS33 } [get_ports { uart_tx_pin }];

# LEDs (Status Monitoring)
...
# SW0 for Dual-Mode Telemetry
set_property -dict { PACKAGE_PIN E22 IOSTANDARD LVCMOS25 } [get_ports { sw_0 }];
